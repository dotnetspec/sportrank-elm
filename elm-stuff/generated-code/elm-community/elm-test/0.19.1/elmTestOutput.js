...
var $author$project$FuzzTests$addOne = function (x) {
	return 1 + x;
};
...
var $elm_explorations$test$Fuzz$extractValid = function (_v0) {
	var a = _v0.a;
	var valid = _v0.b;
	return A2(
		$elm$core$Result$map,
...
var $author$project$FuzzTests$frequencyFuzzer = $elm_explorations$test$Fuzz$frequency(
	_List_fromArray(
		[
			_Utils_Tuple2(
			70,
			$elm_explorations$test$Fuzz$constant(7)),
			_Utils_Tuple2(
			12,
			A2($elm_explorations$test$Fuzz$intRange, 8, 9)),
			_Utils_Tuple2(
			6,
			$elm_explorations$test$Fuzz$constant(6)),
			_Utils_Tuple2(
			9,
			A2($elm_explorations$test$Fuzz$intRange, 2, 4)),
			_Utils_Tuple2(
			1,
			$elm_explorations$test$Fuzz$constant(5)),
			_Utils_Tuple2(
			1,
			$elm_explorations$test$Fuzz$constant(1)),
			_Utils_Tuple2(
			1,
			$elm_explorations$test$Fuzz$constant(10))
		]));
...
var $author$project$FuzzTests$addOneTests = A2(
	...
	_List_fromArray(
		[
			...
			$author$project$FuzzTests$frequencyFuzzer,
			'adds 1 to any integer',
			function (num) {
				return A2(
					$elm_explorations$test$Expect$equal,
					num + 1,
					$author$project$FuzzTests$addOne(num));
			})
		]));
var $author$project$FuzzTests$add = F2(
	function (x, y) {
		return x + y;
	});
...
var $author$project$FuzzTests$addTests = A2(
	$elm_explorations$test$Test$describe,
	'add',
	_List_fromArray(
		[
			...
			$elm_explorations$test$Fuzz$tuple(
				_Utils_Tuple2($elm_explorations$test$Fuzz$int, $elm_explorations$test$Fuzz$int)),
			...
			function (_v0) {
				...
				return A2(
					$elm_explorations$test$Expect$equal,
					num1 + num2,
					A2($author$project$FuzzTests$add, num1, num2));
			})
		]));
...
var $author$project$Example$additionTests = A2(
	$elm_explorations$test$Test$describe,
	'Addition',
	_List_fromArray(
		[
			...
			function (_v1) {
				return A2($elm_explorations$test$Expect$equal, 7, 3 + 4);
			})
		]));
...
var $elm_explorations$test$Fuzz$array = function (fuzzer) {
	return A2(
		$elm_explorations$test$Fuzz$map,
		$elm$core$Array$fromList,
		$elm_explorations$test$Fuzz$list(fuzzer));
};
...
var $author$project$FuzzTests$arrayGetTests = A2(
	$elm_explorations$test$Test$describe,
	'Array.get',
	_List_fromArray(
		[
			A3(
			$elm_explorations$test$Test$fuzz,
			$elm_explorations$test$Fuzz$array(
				A2($elm_explorations$test$Fuzz$intRange, -20, 20)),
			'returns Nothing for out of range index',
			function (intArray) {
				var length = $elm$core$Array$length(intArray);
				return A2(
					$elm_explorations$test$Expect$equal,
					$elm$core$Maybe$Nothing,
					A2($elm$core$Array$get, length, intArray));
			})
		]));
...
var $elm_explorations$test$Expect$lessThan = A2($elm_explorations$test$Expect$compareWith, 'Expect.lessThan', $elm$core$Basics$lt);
...
var $author$project$Example$comparisonTests = A2(
	$elm_explorations$test$Test$describe,
	'Comparison',
	_List_fromArray(
		[
			...
			'2 is not equal to 3',
			function (_v0) {
				return A2($elm_explorations$test$Expect$notEqual, 3, 2);
			}),
			A2(
			$elm_explorations$test$Test$test,
			'4 is less than 5',
			function (_v1) {
				return A2($elm_explorations$test$Expect$lessThan, 5, 4);
			}),
			A2(
			$elm_explorations$test$Test$test,
			'6 is less than or equal to 7',
			function (_v2) {
				return A2($elm_explorations$test$Expect$atMost, 7, 6);
			}),
			A2(
			$elm_explorations$test$Test$test,
			'9 is greater than 8',
			function (_v3) {
				return A2($elm_explorations$test$Expect$greaterThan, 8, 9);
			}),
			A2(
			$elm_explorations$test$Test$test,
			'11 is greater than or equal to 10',
			function (_v4) {
				return A2($elm_explorations$test$Expect$atLeast, 10, 11);
			}),
			A2(
			$elm_explorations$test$Test$test,
			'a list with zero elements is empty',
			function (_v5) {
				return A2(
					$elm_explorations$test$Expect$true,
					'expected the list to be empty',
					$elm$core$List$isEmpty(_List_Nil));
			}),
			A2(
			$elm_explorations$test$Test$test,
			'a list with some elements is not empty',
			function (_v6) {
				return A2(
...
var $elm_explorations$test$MicroRandomExtra$bool = A2(
	$elm$random$Random$map,
	$elm$core$Basics$eq(0),
	A2($elm$random$Random$int, 0, 1));
var $elm_explorations$test$Shrink$bool = function (b) {
	if (b) {
		return A2($elm_explorations$test$Lazy$List$cons, false, $elm_explorations$test$Lazy$List$empty);
	} else {
		return $elm_explorations$test$Lazy$List$empty;
	}
};
var $elm_explorations$test$Fuzz$bool = A2($elm_explorations$test$Fuzz$custom, $elm_explorations$test$MicroRandomExtra$bool, $elm_explorations$test$Shrink$bool);
var $author$project$FuzzTests$flip = function (x) {
	return !x;
};
var $author$project$FuzzTests$flipTests = A2(
	$elm_explorations$test$Test$describe,
	'flip',
	_List_fromArray(
		[
			A3(
			$elm_explorations$test$Test$fuzz,
			$elm_explorations$test$Fuzz$bool,
			'negates the given boolean value',
			function (value) {
				return A2(
					$elm_explorations$test$Expect$equal,
					!value,
					$author$project$FuzzTests$flip(value));
			})
		]));
...
var $author$project$Example$guardianNames = A2(
	$elm_explorations$test$Test$test,
	'only 2 guardians have names with less than 6 characters',
	function (_v0) {
		var guardians = _List_fromArray(
...
var $author$project$FuzzTests$listLengthTests = A2(
	$elm_explorations$test$Test$describe,
	'List.length',
	_List_fromArray(
		[
			A3(
			$elm_explorations$test$Test$fuzz,
			$elm_explorations$test$Fuzz$list($elm_explorations$test$Fuzz$int),
			'never returns a negative value',
			function (intList) {
				return A2(
					$elm_explorations$test$Expect$atLeast,
					0,
					$elm$core$List$length(intList));
			})
		]));
...
var $author$project$FuzzTests$listReverseTests = A2(
	$elm_explorations$test$Test$describe,
	'List.reverse',
	_List_fromArray(
		[
			...
			'doesn\'t remove a member of the list',
			F2(
				function (intList, num) {
					return A2(
						$elm_explorations$test$Expect$equal,
						A2($elm$core$List$member, num, intList),
						A2(
							$elm$core$List$member,
							num,
							$elm$core$List$reverse(intList)));
				}))
		]));
var $elm_explorations$test$Expect$Absolute = function (a) {
	return {$: 'Absolute', a: a};
};
var $elm_explorations$test$Lazy$List$singleton = function (a) {
	return A2($elm_explorations$test$Lazy$List$cons, a, $elm_explorations$test$Lazy$List$empty);
};
var $elm_explorations$test$Shrink$seriesFloat = F2(
	function (low, high) {
		if (_Utils_cmp(low, high - 0.0001) > -1) {
			return (high !== 0.000001) ? $elm_explorations$test$Lazy$List$singleton(low + 0.000001) : $elm_explorations$test$Lazy$List$empty;
		} else {
			var low_ = low + ((high - low) / 2);
			return A2(
				$elm_explorations$test$Lazy$List$cons,
				low,
				A2($elm_explorations$test$Shrink$seriesFloat, low_, high));
		}
	});
var $elm_explorations$test$Shrink$float = function (n) {
	return (n < 0) ? A2(
		$elm_explorations$test$Lazy$List$cons,
		-n,
		A2(
			$elm_explorations$test$Lazy$List$map,
			$elm$core$Basics$mul(-1),
			A2($elm_explorations$test$Shrink$seriesFloat, 0, -n))) : A2($elm_explorations$test$Shrink$seriesFloat, 0, n);
};
var $elm$core$String$fromFloat = _String_fromNumber;
var $elm_explorations$test$Fuzz$floatRange = F2(
	function (lo, hi) {
		return (_Utils_cmp(hi, lo) < 0) ? $elm$core$Result$Err(
			'Fuzz.floatRange was given a lower bound of ' + ($elm$core$String$fromFloat(lo) + (' which is greater than the upper bound, ' + ($elm$core$String$fromFloat(hi) + '.')))) : A2(
			$elm_explorations$test$Fuzz$custom,
			A2(
				$elm_explorations$test$MicroRandomExtra$frequency,
				_Utils_Tuple2(
					8,
					A2($elm$random$Random$float, lo, hi)),
				_List_fromArray(
					[
						_Utils_Tuple2(
						1,
			...
			A2(
				...
				$elm_explorations$test$Shrink$float));
	});
var $author$project$FuzzTests$multiplyFloat = F2(
	function (x, y) {
		return x * y;
	});
...
var $author$project$FuzzTests$multiplyFloatTests = A2(
	$elm_explorations$test$Test$describe,
	'multiplyFloat',
	_List_fromArray(
		[
			A4(
			$elm_explorations$test$Test$fuzz2,
			A2($elm_explorations$test$Fuzz$floatRange, -1.0, 1.0),
			$elm_explorations$test$Fuzz$int,
			...
			F2(
				function (x, y) {
					return A3(
						...
						x * y,
						A2($author$project$FuzzTests$multiplyFloat, x, y));
				}))
		]));
var $elm_explorations$test$Fuzz$float = function () {
	var generator = A2(
		$elm_explorations$test$MicroRandomExtra$frequency,
		_Utils_Tuple2(
			3,
			A2($elm$random$Random$float, -50, 50)),
		_List_fromArray(
			[
				_Utils_Tuple2(
				0.5,
				...
				_Utils_Tuple2(
				1,
				A2($elm$random$Random$float, 0, $elm$random$Random$maxInt - $elm$random$Random$minInt)),
				_Utils_Tuple2(
				1,
				A2($elm$random$Random$float, $elm$random$Random$minInt - $elm$random$Random$maxInt, 0))
			]));
	return A2($elm_explorations$test$Fuzz$custom, generator, $elm_explorations$test$Shrink$float);
}();
var $elm_explorations$test$Fuzz$percentage = function () {
	var generator = A2(
		$elm_explorations$test$MicroRandomExtra$frequency,
		_Utils_Tuple2(
			8,
...
var $author$project$FuzzTests$pizzaLeft = F2(
	function (eatenPercent, totalSlices) {
		return totalSlices - (eatenPercent * totalSlices);
	});
var $author$project$FuzzTests$pizzaLeftTests = A2(
	$elm_explorations$test$Test$describe,
	'pizzaLeft',
	_List_fromArray(
		[
			A4(
			$elm_explorations$test$Test$fuzz2,
			$elm_explorations$test$Fuzz$percentage,
			$elm_explorations$test$Fuzz$float,
			'returns remaining pizza slices',
			F2(
				function (eaten, total) {
					return A3(
						$elm_explorations$test$Expect$within,
						$elm_explorations$test$Expect$Absolute(0.000000001),
						total - (eaten * total),
						A2($author$project$FuzzTests$pizzaLeft, eaten, total));
				}))
		]));
...
var $author$project$FuzzTests$stringTests = A2(
	$elm_explorations$test$Test$describe,
	'The String module',
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					A2(
					$elm_explorations$test$Test$test,
					'has no effect on a palindrome',
					function (_v0) {
						var palindrome = 'hannah';
						return A2(
							$elm_explorations$test$Expect$equal,
							palindrome,
							$elm$core$String$reverse(palindrome));
					}),
					A2(
					$elm_explorations$test$Test$test,
					'reverses a known string',
					function (_v1) {
						return A2(
							$elm_explorations$test$Expect$equal,
							'GFEDCBA',
							$elm$core$String$reverse('ABCDEFG'));
					}),
					A3(
					$elm_explorations$test$Test$fuzz,
					...
					function (randomlyGeneratedString) {
						return A2(
							...
							$elm$core$String$reverse(
								$elm$core$String$reverse(randomlyGeneratedString)));
					})
				]))
		]));
var $author$project$Test$Generated$Main1133415622$main = A2(
	$author$project$Test$Runner$Node$run,
	{
		paths: _List_fromArray(
			['/home/linuxuser/Documents/dev/elm/project1/tests/FuzzTests.elm']),
		processes: 4,
		report: $author$project$Test$Reporter$Reporter$ConsoleReport($author$project$Console$Text$UseColor),
		runs: $elm$core$Maybe$Nothing,
		seed: 325516428174872
	},
	$elm_explorations$test$Test$concat(
		_List_fromArray(
			[
				...
				'Example',
				_List_fromArray(
					[$author$project$Example$guardianNames, $author$project$Example$comparisonTests, $author$project$Example$additionTests])),
				A2(
				$elm_explorations$test$Test$describe,
				'FuzzTests',
				_List_fromArray(
					[$author$project$FuzzTests$addOneTests, $author$project$FuzzTests$addTests, $author$project$FuzzTests$arrayGetTests, $author$project$FuzzTests$flipTests, $author$project$FuzzTests$listLengthTests, $author$project$FuzzTests$listReverseTests, $author$project$FuzzTests$multiplyFloatTests, $author$project$FuzzTests$pizzaLeftTests, $author$project$FuzzTests$stringTests]))
			])));
_Platform_export({'Test':{'Generated':{'Main1133415622':{'init':$author$project$Test$Generated$Main1133415622$main($elm$json$Json$Decode$int)(0)}}}});}(this));
return this.Elm;
})({});
var pipeFilename = "/tmp/elm_test-12044.sock";
